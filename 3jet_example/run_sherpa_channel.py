...
#        p1, p2 = self.ChannelIso(rans,pa,pb)
        ...
        #GenerateMomentaVec = np.vectorize(self.GenerateMomenta, excluded=(1,2,3))
        #p1, p2, p3 = GenerateMomentaVec(channel,rans,pa,pb)

        ch1roll = np.rollaxis(np.asarray(self.channels[0](rans,pa,pb)), axis=1)
        ch2roll = np.rollaxis(np.asarray(self.channels[1](rans,pa,pb)), axis=1)
        channel_large = np.reshape(np.outer(channel,np.ones((3,4))),(-1,3,4))
        p1, p2, p3 = np.rollaxis(np.where(channel_large == 0,ch1roll,ch2roll), axis=1)
...
if __name__ == '__main__':
    ...
    from integrator_channel import *
    ...
    def func_int(x,channel):
        channel=np.asarray(channel['channel'])
        ret = tf.where(channel ==0,tf.stop_gradient(tf.py_function(hardxs.GeneratePointCh,[x,0],tf.float32)),tf.stop_gradient(tf.py_function(hardxs.GeneratePointCh,[x,1],tf.float32)))
        return ret
...
#        return tf.concat([channel1,channel2],0)
    ...
    import corner
    import os.path
    if os.path.exists('matrix_channel.pdf')==False:
        print(" * * * Plotting Matrixelement * * * ")
        nsamp = 100000
        x = np.random.random((nsamp,5))
        ran = np.random.randint(2,size = nsamp)

        # test to run single channel:
        #ran = 0 * np.ones((nsamp,))

        #hardxs.channel = 1
        p = hardxs.GeneratePointCh(x,ran)
        ...
        plt.savefig('matrix_channel.pdf')
    else:
