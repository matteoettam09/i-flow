...
import numpy as np
import flow.piecewise_tf20 as piecewise
...
class Integrator():
    ...
    def __init__(self, func, ndims, layers=4,
        ...
        for i in range(ndims):
            ...
            if i == 0:
                self.trainable_variables = self.bijectors[-1].trainable_variables
            else:
                self.trainable_variables.extend(self.bijectors[-1].trainable_variables)
        ...
        self._make_optimizer(learning_rate=learning_rate)
...
#                var/nsamples, x, p, q)
    ...
    def _make_optimizer(self, learning_rate=1e-4):
    ...
    def train_one_step(self,nsamples,alpha):
        with tf.GradientTape() as tape:
            (loss, integral, var, x, p, q) = self._loss_fn(nsamples, alpha)
    ...
    def optimize(self, **kwargs):
        """Optimize the integrator for the given function.
...
#                    min_loss = np_loss
    ...
    def integrate(self, nsamples=10000, plot=False, acceptance=False, **kwargs):
        ...
        if acceptance:
            ...
            np.savetxt(os.path.join(path,'weights{}.txt'.format(fname)),wgt,delimiter='\n')

            plt.hist(wgt, bins=np.logspace(np.log10(min_val),
                                                   np.log10(max_val),
            ...
            plt.close()
            return integral.numpy(), error.numpy(), np.mean(wgt)/np.max(wgt)

        return integral.numpy(), error.numpy()
