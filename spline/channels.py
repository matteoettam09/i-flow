...
class SChannelDecay:
    ...
    def _find_axes(self,p):
        # z-axis along beam:
        return Vector4(np.zeros_like(p[:,0]),np.zeros_like(p[:,1]),np.zeros_like(p[:,2]),np.ones_like(p[:,3])), \
            Vector4(np.zeros_like(p[:,0]),np.zeros_like(p[:,1]),np.ones_like(p[:,2]),np.zeros_like(p[:,3])), \
            Vector4(np.zeros_like(p[:,0]),np.ones_like(p[:,1]),np.zeros_like(p[:,2]),np.zeros_like(p[:,3]))
        ## z-axis along boost:
